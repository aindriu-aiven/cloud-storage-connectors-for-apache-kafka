...
public interface TaskAssignment {
    ...
    boolean isPartOfTask(String valueToBeEvaluated);

    void configureTask(int taskId, int maxTasks, String expectedFormat);
    ...
    default boolean taskMatchesPartition(final int taskId, final int partitionId) {
        // The partition id and task id are both expected to start at 0 but if the task id is changed to start at 1 this will break.
